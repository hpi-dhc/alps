...
class RawFileParserTask(Task):
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        for signal_name, data in retval.items():
            ...
            if signal_type in [signal_types.NN_INTERVAL, signal_types.RR_INTERVAL]:
                self._save_to_sample_table(series, signal)
            elif signal_type is signal_types.TAGS:
                self._save_to_tag_table(series, signal)
            else:
                self._save_to_signal_file(series, signal)
    ...
    def _save_to_sample_table(self, series, signal):
        samples = [
            Sample(
                timestamp=index,
                value=value,
    ...
    def _save_to_tag_table(self, series, signal):
        tags = [
            Tag(
                timestamp=index,
                value=value,
    ...
    def _save_to_signal_file(self, series, signal):
        lower_bound = series.index.min()
        lower_bound = lower_bound - pd.Timedelta(
            microseconds=lower_bound.microsecond
        )
        upper_bound = lower_bound + pd.Timedelta(seconds=self.CHUNK_LENGTH)
...
def parse_raw_files(file_ids, dataset_id):
    ...
    # returned result of celery task has to be json serializable
    for each in result.values():
        each['series'] = each['series'].dropna().to_json()
    return result
